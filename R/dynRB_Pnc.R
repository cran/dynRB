...
function(A=A,steps=201,graphic=FALSE){
  ...
  if(graphic){
    for(i in 1:nrow(G)){
      ...
      EE<-funclist4$portionAinB_coordinates_full_adjusted_graphics(S1,S2,steps=steps0)
    ...
    for(i in 1:nrow(G)){
      ...
      EE<-funclist4$portionAinB_coordinates_full_adjusted_graphics_off(S1,S2,steps=steps0)
      G[i,3:(dims+2)]<-EE$integral_coord
      print(i)
    }
  }
